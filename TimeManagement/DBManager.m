...
-(void)runQuery:(const char *)query isQueryExecutable:(BOOL)queryExecutable{
    ...
    if (openDatabaseResult == SQLITE_OK) {
        // Declare a sqlite3_stmt object in which will be stored the query after having been compiled
        ...
        if (prepareStatementResult == SQLITE_OK) {
            ...
            if (!queryExecutable) {
                ...
                if (executeQueryResults == SQLITE_DONE) {
                    ...
                    // If it could not execute the query show the error message on the debugger.
                    NSLog(@"DB Error: %s", sqlite3_errmsg(sqlite3Database));
                }
            }
        }
        else {
            // If the database cannot be opened then show the error message
